...
class MvsecFlow(Dataset):
    def __init__(self, args, filters, type, path):
        ...
        self.dataset = self.get_indices(path, args['datasets'], args['align_to'], filters)
    ...
    def get_indices(self, path, dataset, align_to, filters):
        ...
        for dataset_name in dataset:
            ...
            for subset in dataset[dataset_name]:
                ...
                subset_filters = filters[dataset_path]
    ...
    def get_data_sample(self, loader_idx):
        ...
        return_dict = {'idx': idx,
                       ...
                       'gt_valid_mask': torch.from_numpy(flow_valid),
                       'gt_valid_mask_stack': torch.from_numpy(numpy.stack([flow_valid] * 2, axis=0)),
                       "param_evc": {'height': self.image_height,
                                     'width': self.image_width},
                       "dataset_name": int(subset)
                       }
        ...
        if input_type == 'events':
            ...
            # print('type(events_new):', type(events_new.to_numpy()))  type(events_new): <class 'numpy.ndarray'>
            return_dict['events'] = torch.from_numpy(events_new.to_numpy())
    ...
    def get_events(self, loader_idx):
        ...
        path_dataset = os.path.join(self.path_dataset, self.dataset[loader_idx]['dataset_name'] + "_" + str(
            self.dataset[loader_idx]['subset_number']))
        params = {'height': self.image_height, 'width': self.image_width}
    ...
    def __getitem__(self, idx, force_crop_window=None, force_flipping=None):
        ...
        sample['flow_gt'] = self.cropper(sample['flow'])
        sample['valid_2D'] = self.cropper(sample['gt_valid_mask'])
        sample['valid_2D_stack'] = self.cropper(sample['gt_valid_mask_stack'])
