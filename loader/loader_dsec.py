...
class Sequence(Dataset):
    def __init__(self, seq_path: Path, representation_type: RepresentationType, mode: str = 'test',
        ...
        test_timestamp_file = flow_dir / 'test_timestamps.txt'
        assert test_timestamp_file.is_file()
        file = np.genfromtxt(
            test_timestamp_file,
            delimiter=','
        )
        self.start_times = file[:, 0]
        self.end_tiems = file[:, 1]
        #  self.idx_to_visualize = file[:, 2]  # csv文件中的file_index
        ...
        assert len(self.flow_gt_pathstring) == self.start_times.size
    ...
    def get_data_sample(self, index, crop_window=None, flip=None):
        ...
        ts_start = [self.start_times[index] - self.delta_t_us, self.start_times[index]]
        ts_end = [self.end_tiems[index] - self.delta_t_us, self.end_tiems[index]]

        file_index = int(self.indices[index])

        flow_gt_path = Path(self.flow_gt_pathstring[index])
        flow, valid_2D = self.load_flow(flow_gt_path)

        output = {'timestamp': self.start_times[index],
                  'file_index': file_index,
                  'save_submission': self.visualize_samples,
...
class SequenceRecurrent(Sequence):
    ...
    def get_continuous_sequences(self):
        ...
        if self.sequence_length > 1:
            for i in range(len(self.start_times) - self.sequence_length + 1):
                diff = self.start_times[i + self.sequence_length - 1] - self.start_times[i]
            ...
            for i in range(len(self.start_times) - 1):
                diff = self.start_times[i + 1] - self.start_times[i]
                if diff < np.max([100000 * (self.sequence_length - 1) + 1000, 101000]):
                    continuous_seq_idcs.append(i)
