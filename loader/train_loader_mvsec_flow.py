...
class MvsecFlow(Dataset):
        ...
        for dataset_name in dataset:
            ...
            for subset in dataset[dataset_name]:
                ...
                if align_to.lower() == 'images' or align_to.lower() == 'image':
                    ...
                    self.update_rate = 45
                    self.timestamp_files_flow[dataset_name][subset] = numpy.loadtxt(os.path.join(flow_path,
                                                                                                 dataset_path,
                ...
                ts = numpy.loadtxt(os.path.join(flow_path, dataset_path, ts_path))
    ...
    def estimate_gt_flow(self, loader_idx, ts_old, ts_new):
        ...
        # print('ts_old:{}, path_flow is {}.'.format(ts_old, path_flow))
    ...
    def __getitem__(self, idx, force_crop_window=None, force_flipping=None):
        ...
        flow = self.cropper(sample['flow'])
        gt_valid_mask = self.cropper(sample['gt_valid_mask'])
        event_volume_new = self.cropper(sample['event_volume_new'])
        event_volume_old = self.cropper(sample['event_volume_old'])
        loader_idx = sample['loader_idx']

        return flow, gt_valid_mask, event_volume_old, event_volume_new, loader_idx
