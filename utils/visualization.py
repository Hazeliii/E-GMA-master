...
import numpy
import os
import loader.loader_utils as loader
...
class BaseVisualizer(object):
    def __init__(self, dataloader, save_path, additional_args=None):
        ...
        self.submission_path = helper.create_save_path(save_path, 'submission')
        self.mode = 'test'
    ...
    def visualize_flow_colours(self, flow, true_idx, dataset_name, epoch=None, data_aug=False, is_gt=False, fix_scaling=10,
                               custom_name=None, prefix=None, suffix=None, sub_folder=None):
        true_idx = int(true_idx)

        if custom_name is None:
            name = TEMPLATES.FLOW_TEST.format('inference', dataset_name, true_idx)
        else:
            name = custom_name
...
class FlowVisualizerEvents(BaseVisualizer):
    ...
    def visualize_ground_truths(self, batch, batch_idx, epoch=None, data_aug=False):
        ...
        '''
        # Visualize Flow GT
        flow_gt = batch['flow_gt'][batch_idx].clone()
        flow_gt[~batch['valid_2D_stack'][batch_idx].bool()] = 0.0
        self.flow_scaling = self.visualize_flow_colours(flow_gt, batch['idx'][batch_idx],
                                                        dataset_name=int(batch['dataset_name'][batch_idx]), epoch=epoch,
    ...
    def visualize_estimations(self, batch, batch_idx, epoch=None):
        ...
        if self.clamp_flow:
            ...
            scaling = None
        dataset_name = int(batch['dataset_name'][batch_idx])
        self.visualize_flow_colours(batch['flow_est'][batch_idx], batch['idx'][batch_idx], dataset_name, epoch=epoch,
                                    is_gt=False, fix_scaling=scaling)

        # Visualize Masked Flow
        flow_est = batch['flow_est'][batch_idx].clone()
        flow_est[~batch['valid_2D_stack'][batch_idx].bool()] = 0.0
        self.visualize_flow_colours(flow_est, batch['idx'][batch_idx],
                                    dataset_name=dataset_name, epoch=epoch,
