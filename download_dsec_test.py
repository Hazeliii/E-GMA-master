...
if __name__ == '__main__':
    ...
    for seq_name in TEST_SEQUENCES:
        ...
        if not test_timestamps_file_destination.exists():
            shutil.move(test_timestamps_dir / (seq_name + '.csv'), test_timestamps_file_destination)  # move(old, new)  old->new
        ...
        if not (events_left_file.parent / events_left_file.stem).exists():
            assert download(events_left_url, events_left_file), events_left_url    # stem 目录最后一个部分，没有后缀
            unzip(events_left_file)

    shutil.rmtree(test_timestamps_dir)  # 删除
