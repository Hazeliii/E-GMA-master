...
def gen_h5_file_partly(event_times_pth, events_pth, image_time_pth, saved_pth):
    ...
    for i in range(start_i+1, len(image_times)):  # 总共的.h5文件，与image_times相同
...
if __name__ == '__main__':
    ...
    for name in sequence_name:
        ...
        events_time_pth = os.path.join(saved_root, name, 'event_times.txt')
        events_txt_pth = os.path.join(saved_root, name, 'events.txt')
        if not os.path.exists(saved_event_root):
            os.makedirs(saved_event_root)
        read_events(event_pth, events_txt_pth)
        gen_h5_file_partly(events_time_pth, events_txt_pth, image_time_pth, saved_event_root)
