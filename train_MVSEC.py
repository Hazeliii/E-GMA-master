...
from motion_compensation_loss_func.compensation_loss import motion_compensation_losses
sys.path.append('core')

os.environ["CUDA_VISIBLE_DEVICES"] = "3, 1 "  # 4和5
...
def motion_compensation_loss(flow_pred_list, events_list, flow_gt, valid_gt, smoothness_weight=0.5):
    """
    基于运动补偿的无监督损失函数，从motion_compensation_loss_func.compensation_loss中选择
    @param flow_pred_list: N*B*2*H*W
    @param events_list:B*event_set, every event set consists of N*(t,x,y,p)
    @param flow_gt:B*2*H*W
    @param valid_gt:B*H*W
    @return:
    """
    # print('In motion_compensation_loss .')
    flow_pred_last = flow_pred_list[-1]  # (B, 2, H, W)
    batch = flow_pred_last.shape[0]
    flow_transposed = flow_pred_last.permute(0, 2, 3, 1)
    warp_loss = 0
    device = flow_pred_last.device
    for i in range(batch):
        loss_func = motion_compensation_losses(torch.tensor(events_list[i], device=device), flow_pred_last[i], device)
        warp_loss += loss_func.contrast_max_based_loss()
    smooth_loss = compute_smooth_loss(flow_transposed)
    loss = warp_loss + smooth_loss * smoothness_weight

    flow_gt = flow_gt.permute(0, 2, 3, 1)   # B*H*W*2
    gt_masked = flow_gt[valid_gt, :]
    flow_last_masked = flow_transposed[valid_gt, :]
    ...
    # order: AEE   loss   n_points  outlier  smooth_loss  warp_loss
    return loss, metrics
...
def train(args):
    ...
    model = nn.DataParallel(ERAFT(args, config['data_loader']['train']['args']['num_voxel_bins']).cuda(),
    ...
    while should_keep_training:
        for i_batch, data_blob in enumerate(train_set_loader):
            optimizer.zero_grad()
            gt_flow, valid_2D, vox_old, vox_new, loader_idx = [x.cuda() for x in data_blob]
            # 每一次迭代都有一次flow_pre,则flow_pre为n*b*w*h*2
            delta_f, flow_prediction = model(vox_old, vox_new)
            events_list = list()
            idx_list = list()
            # print('loader_idx:', loader_idx) tensor([32132, 18804,  5585, 21657], device='cuda:0')
            for idx in loader_idx.cpu().numpy().astype(int):
                idx_list.append(idx)
                events = train_set.get_events(idx)
                events_list.append(events)
            # supervised loss: sequence_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='att_SE_voxel', help="name your experiment")
    ...
    parser.add_argument('--output', default='checkpoints_mvsec_outdoor')
    parser.add_argument('--config_path', type=str, default='config/train_mvsec_45_outdoor.json')
    parser.add_argument('--filters_json', type=str, default='config/mvsec_train_outdoor_filter.json')
